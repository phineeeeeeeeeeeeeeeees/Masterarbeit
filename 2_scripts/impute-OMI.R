...
# =====================================
# CAMS-Reanalysis NO and NO2 (0.75˚*0.75˚)
# 3D array: x, y, time 
...
# only the grids
{
  ggplot() +
    ...
    labs(x = "Longitude" , y = "Latitude" , 
         ...
         subtitle = "Pink: original CAMS grids; Blue: CAMS resampled to OMI grids") +
    theme_bw()
}
...
#save(gridSearch_hour_var , file = "3_results/output-data/imputation-OMI/gridSearch_hour_var_spatialGridCv.R")
...
# gridSearch_hour_var$HOUR_15$base_meteo5$varImportance %>% 
#   as.data.frame() %>% 
...
# =====================================
# model development: grid search for final model (fixed hour, different variable combinations)
# =====================================
modelInput_df <- imputation_df %>%
  select(x , y , DOY , OMI_NO2 , DEM , ends_with("_12") , ends_with("_15") , spatial_CV) %>%
  # exclude rows with missing predictor/response value
  filter(!if_any(everything() , is.na)) %>%
...
rf_12H <- ranger(OMI_NO2 ~ x + y + DOY + DEM + CAMS_NO2_12 + CAMS_NO_12 + 
                   blh_12 + t2m_12 + sp_12 + tcc_12 + tp_12 + R1 + R2 + R3 + wd_12 + ws_12 , 
                 data = modelInput_df ,
                 importance = "impurity" ,
                 keep.inbag = TRUE)
rf_12H
importance(rf_12H) %>%
  ...
  geom_bar(stat = "identity") +
  coord_flip()
# total precipitation: likely to be useless
...
# grid search: exclude i least important variables at a time
{
  ...
  for(i in gridSearch_i){
    ...
    formula_rf.temp <- sprintf("OMI_NO2 ~ %s" , paste(inputVar_ordered[1:(length(inputVar_ordered)-i+1)] , collapse = '+')) %>% 
...
#save(gridSearch_12H , file = "3_results/output-data/imputation-OMI/gridSearch_12H.RData")
...
# time series plot of one pixel
spatialtemporal[,12,7,] %>% 
  as.data.frame() %>% 
  select(x , y , DOY , OMI_NO2 , ends_with("_12")) %>% 
  mutate(across(everything() , as.numeric)) %>% 
  pivot_longer(cols = -c(x,y,DOY) , names_to = "var") %>% 
...
# decided to use num.trees = 1000 and mtry = 5
...
rf_final_indices %>% 
  ...
  mutate(across(c(value , min , max) , format , trim = TRUE , digits = 3 , scientific = FALSE)) %>% 
  bind_rows(
    rf_final_indices %>% 
      filter(str_detect(name , "intercept")) %>% 
      # value format: with ranges
      mutate(across(c(value , min , max) , format , trim = TRUE , digits = 3 , scientific = TRUE))
  ) %>% 
  mutate(value = glue::glue_data(. , "{value} ({min}~{max})") %>% 
           str_remove_all("\\(NA~NA\\)") %>% str_trim()) %>% 
  select(name , value) %>% 
  pivot_wider(names_from = "name" , values_from = "value") %>% 
  select(OOB_R2 , OOB_MAPE , OOB_slope , OOB_intercept , CV_R2 , CV_MAPE , CV_slope , CV_intercept) %>% 
  write.csv("3_results/output-data/imputation-OMI/indices.csv")
...
# residuals-- distribution pattern
projected_stars %>%
  mutate(residual = OMI_NO2 - OMI_NO2_pred) %>%
...
ggplot() +
  ...
  facet_wrap(~time) +
  coord_sf(expand = FALSE) +
  scale_fill_gradient2(limits = c(-5e14 , 5e14) ,
...
# library(spdep)
# # distance matrix
# distMtrx <- rf_final_obs_pred %>% 
#   select(x,y,DOY) %>% 
#   mutate(across(everything() , scale)) %>% 
#   #slice(1:100) %>% 
#   as.matrix() %>% 
#   dist()
# # convert distance matrix to weight object
# distW <- mat2listw(as.matrix(distMtrx) , style = "W")
# # Moran's I test
# moranTest_resid <- rf_final_obs_pred %>% 
#   transmute(resid = obs - pred) %>% 
#   #slice(1:100) %>% 
#   unlist() %>% 
#   moran.test(. , listw = distW , zero.policy = TRUE)
# # plot the ACF/PACF plot of the time-series of one pixel
# projected_stars[,10,6,] %>% 
#   mutate(residual = OMI_NO2 - OMI_NO2_pred) %>% 
#   select(residual) %>% 
#   pull() %>% 
#   c() %>% 
...
anim_save(filename = "3_results/output-data/imputation-OMI/obs_pred_imputed.gif")


#save(list = ls() , file = "3_results/output-data/imputation-OMI/working-environment.RData")
