...
rf_final_obs_pred <- modelInput_df %>% 
  select(x,y,DOY,TROPOMI_NO2,spatial_CV) %>% 
  rename(obs = TROPOMI_NO2) %>% 
...
for(k in 1:k_fold){
  ...
  if(k == 1){
    ...
    rf_final_obs_pred_cv <- rf_final_obs_pred_cv %>% 
      ...
      select(x,y,DOY,TROPOMI_NO2,spatial_CV,pred_CV)
  }
  # clean
...
rf_final_obs_pred <- rf_final_obs_pred %>% 
  inner_join(rf_final_obs_pred_cv , by = c("x" , "y" , "DOY" , "spatial_CV" , "obs"="TROPOMI_NO2"))
...
rf_final_indices %>% 
  ...
  write.csv("3_results/output-data/imputation-TROPOMI/indices.csv")
...
cowplot::save_plot(
  filename = "3_results/output-data/imputation-TROPOMI/obs_pred_scatterplot.png" , 
  plot = last_plot() , 
